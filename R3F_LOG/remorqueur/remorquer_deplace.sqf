...
else
{
	...
	if (!isNull _remorqueur && {
		...
		alive _remorqueur && isNull (_remorqueur getVariable "R3F_LOG_remorque") && (vectorMagnitude velocity _remorqueur < 6) && VEHICLE_UNLOCKED(_remorqueur) && !(_remorqueur getVariable "R3F_LOG_disabled")
	}) then
	{
