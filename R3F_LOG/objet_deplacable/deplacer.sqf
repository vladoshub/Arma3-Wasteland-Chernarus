...
_poiMarkers = allMapMarkers select {markerType _x isEqualTo "Empty" && {[["VehStore","Mission_","ForestMission_","LandConvoy_","BigBoat_","UltraMission_", "UnlockBuildBase"], _x] call fn_startsWith}};
if (({ (getPosASL player) vectorDistance (ATLtoASL getMarkerPos _x) < _poiDist } count _poiMarkers > 0) && ({ (([["UnlockBuildBase"], _x] call fn_startsWith) && ((getPosASL player) vectorDistance (ATLtoASL getMarkerPos _x) < 400)) } count _poiMarkers == 0))  exitWith
	{
		player globalChat "You are not allowed to moving objects within stores and mission spawns";
	};
*/
