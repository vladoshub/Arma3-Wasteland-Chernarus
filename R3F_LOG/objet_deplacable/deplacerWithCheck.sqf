...
else
{
	...
	_currentAnimRLog =	animationState player;
	_configRLog = configFile >> "CfgMovesMaleSdr" >> "States" >> _currentAnimRLog;
	_onLadderRLog =	(getNumber (_configRLog >> "onLadder"));
	...
	if(_onLadderRLog == 1) exitWith { 
		R3F_LOG_mutex_local_verrou = false;
		player globalChat "You can't move this object while on a ladder";
	};
	...
	if ( (({ (getPosASL player) vectorDistance (ATLtoASL getMarkerPos _x) < _poiDist } count _poiMarkers > 0)) && ({ (([["UnlockBuildBase"], _x] call fn_startsWith) && ((getPosASL player) vectorDistance (ATLtoASL getMarkerPos _x) < 400)) } count _poiMarkers == 0))  exitWith
		{
			R3F_LOG_mutex_local_verrou = false;
			player globalChat format ["You are not allowed to moving objects within %1m of stores or mission spawns or ATMs or Parkings. ONLY LOADING IN THE VEHICLE", _poiDist];
		};


		if(_enemyCount > 0 && !(_objet isKindOf "StaticWeapon" || unitIsUAV _objet)) exitWith {
			R3F_LOG_mutex_local_verrou = false;
			player globalChat "You can't move this object when enemy near";
		};
		...
		if (isNull (_objet getVariable ["R3F_LOG_remorque", objNull])) then
		{
			if (count crew _objet == 0 || getNumber (configFile >> "CfgVehicles" >> (typeOf _objet) >> "isUav") == 1) then
			{
				...
				if (_objet getVariable "R3F_LOG_est_deplace_par" == _joueur && isNull (_objet getVariable ["R3F_LOG_est_transporte_par", objNull])) then
				{
					...
					while {!isNull R3F_LOG_joueur_deplace_objet && _objet getVariable "R3F_LOG_est_deplace_par" == _joueur && alive _joueur && ({ (isPlayer _x && alive _x && _x distance player < 30) && !([_x, player] call A3W_fnc_isFriendly) } count playableUnits) < 1 && { (_x getVariable ["is_base_flag_activate", false]) && ( !( (_x getVariable ["ownerUID","0"] isEqualTo getPlayerUID player) || (_x getVariable ["ownerUID", "0"] in ((units player) apply {getPlayerUID _x})) || (group _x == group player) || ((side _x == side player) && (str(side player) == "WEST" || str(side player) == "EAST")) || ((str(side player) == "WEST" || str(side player) == "EAST") && (str(side player)) == (_x getVariable ["LastSide", ""]))) || ((str(side player)) != (_x getVariable ["LastSide", (str(side player))]))) } count (player nearObjects ["FlagChecked_F", 320]) < 1 && ({(isPlayer _x && alive _x && _x distance _objet < 100 && _x getVariable "mutex_net_obj" isEqualTo netId _objet)} count (playableUnits) < 1) && !(_objet getVariable ["objectLocked", false])} do
					{
