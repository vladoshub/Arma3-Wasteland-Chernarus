...
else
{
	...
	_objet = objNull;
	{
		if (
			...
			_x != _heliporteur && VEHICLE_UNLOCKED(_x) && !(_x getVariable "R3F_LOG_disabled") &&
	...
	if (!isNull _objet) then
	{
		...
		{
		if !(_objet getVariable ["objectLocked", false]) then {
				...
				//{
					...
					if (isNull (_objet getVariable ["R3F_LOG_remorque", objNull])) then
					{
						...
						while {_attente_valide && time - _time_debut < _duree} do
						{
							...
							private _isUav =  (getNumber (configFile >> "CfgVehicles" >> (typeOf _objet) >> "isUav") == 1);
							...
							if !(
								...
								((count crew _objet == 0 || _isUav)) &&
								!(_objet getVariable ["objectLocked", false]) &&
								({(isPlayer _x && alive _x && _x distance _objet < 100 && _x getVariable "mutex_net_obj" isEqualTo netId _objet)} count (playableUnits) < 1)
							) then
							{
...

						{
							if (isPlayer _x && alive _x && _x distance _objet < 100) then
								{
								if (_x getVariable "mutex_net_obj" isEqualTo netId _objet && _objet getVariable ["objectLocked", false]) then
									{
										_isBusy = true;
									};
								};
						...
						if (_attente_valide && !_isBusy) then
						{
			...
			player globalChat "object is locked!";
		};
		};
	};
