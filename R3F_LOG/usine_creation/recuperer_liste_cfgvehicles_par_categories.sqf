...
for [{_idx_config = 0}, {_idx_config < _nb_config}, {_idx_config = _idx_config+1}] do
{
	...
	//if (isClass _config && (getText (configFile >> "CfgVehicles" >>  configName _config >> "displayName")) in R3F_LOG_CFG_CF_whitelist_objects) then
	if (isClass _config && (configName _config) in R3F_LOG_CFG_CF_whitelist_objects) then
	{
