	...
	if (_classe != "") then
	{
		...
		if (_usine getVariable "R3F_LOG_CF_credits" == -1 || _usine getVariable "R3F_LOG_CF_credits" >= _cout) then
		{
			...
			if (_classe isKindOf "AllVehicles") then
			{
				...
				if(count _pos_degagee < 1) then {
					_pos_degagee = player modelToWorldWorld [0,-14,0];
					_pos_degagee set [2, (_pos_degagee select 2) + 2];
					_NONE = true;
				} else {
				_pos_degagee set [2, (getPosATL player select 2) + 1]; //BY VLADOS AIR FIX 
				};
			}
			else
			{
			...
			if (count _pos_degagee > 0) then
			{
				...
				if (_action_confirmee) then
				{
					...
					private _money = player getVariable [_payType, 0];
 				    if (_cout > _money) then {
						player globalChat "You don't have money for buy";
						playSound "FD_CP_Not_Clear_F";
						R3F_LOG_mutex_local_verrou = false;
 				    } else {

					if(_NONE) then {
						//_objet = _classe createVehicle [0,0,0];
						//_objet setVehiclePosition [_pos_degagee, [], 0, "NONE"];
						_objet = createVehicle [_classe, _pos_degagee, [], 0, "NONE"];
						[[_objet],"Create_Vehicle_R3F_LOG",false,false,false] call BIS_fnc_MP;
					} else {
						_objet = _classe createVehicle _pos_degagee;
						[[_objet],"Create_Vehicle_R3F_LOG",false,false,false] call BIS_fnc_MP;
						_objet setPosATL _pos_degagee;
					};
					...
					if !(isNull _objet) then
					{
						...
						missionNamespace setVariable ["R3F_LOG_factory_lastCraftTime", diag_tickTime];
							...
							{ 
								_objet setVariable ["water", 50, true];
							}; 
							if (_classe == "Land_Sacks_goods_F") then
							{ 
								_objet setVariable ["food", 40, true];
							};

							if (_classe != "Land_Sacks_goods_F" && _classe != "Land_BarrelWater_F" && !(_objet isKindOf "ReammoBox_F")) then
							{ 
								_objet setVariable ["allowDamage", true, true];
								_objet allowDamage true;
							};

							_objet setVariable ["A3W_purchasedStoreObject", true];
							_objet setVariable ["ownerUID", getPlayerUID player, true];
							_objet setVariable ["ownerName", name player, true];

							private _variant = "";
							if (_variant != "") then
							{
								_objet setVariable ["A3W_vehicleVariant", _variant, true];
							};
						...
						if (getNumber (configFile >> "CfgVehicles" >> (typeOf _objet) >> "isUav") == 1) then
						{
							...
							private _grp = [_objet, _playerSide, true] call fn_createCrewUAV;

							if (isNull (_objet getVariable ["ownerGroupUAV", grpNull])) then
							{
								_objet setVariable ["ownerGroupUAV", _playerGroup, true];
							};
						};
						...
						if (!(_objet isKindOf "AllVehicles") || _est_deplacable) then
						{
							...
							[_objet, player, 0, true] spawn R3F_LOG_FNCT_objet_deplacer_with_check_enemy;
						}
						else
						{
						...
						if(_payType isEqualTo "bmoney") then {
							private _newBalance = _money - _cout;
							player setVariable ["bmoney", _newBalance, true];
						} else {
							[player, -_cout] call A3W_fnc_setCMoney;
						};
