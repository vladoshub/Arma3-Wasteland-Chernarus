...
while {true} do
{
	...
	) then
	{
		...
		R3F_LOG_action_deplacer_objet_valide_enemy = (({ (isPlayer _x && alive _x && _x distance player < 20) && !([_x, player] call A3W_fnc_isFriendly) } count playableUnits) < 1 && { (_x getVariable ["is_base_flag_activate", false]) && ( !( (_x getVariable ["ownerUID","0"] isEqualTo getPlayerUID player) || (_x getVariable ["ownerUID", "0"] in ((units player) apply {getPlayerUID _x})) || (group _x == group player) || ((side _x == side player) && (str(side player) == "WEST" || str(side player) == "EAST")) || ((str(side player) == "WEST" || str(side player) == "EAST") && (str(side player)) == (_x getVariable ["LastSide", ""])) ) || ((str(side player)) != _x getVariable ["LastSide", (str(side player))]) ) } count (player nearObjects ["FlagChecked_F", 320]) < 1);

		R3F_LOG_action_deplacer_objet_valide_not_busy = (({(isPlayer _x && alive _x && _x distance _objet_pointe < 100 && _x getVariable "mutex_net_obj" isEqualTo netId _objet_pointe)} count (playableUnits) < 1) && !(_objet_pointe getVariable ["objectLocked", false]));
		...
		if (_fonctionnalites select __can_be_moved_by_player) then
		{
			...
			R3F_LOG_action_deplacer_objet_valide = (count crew _objet_pointe == 0 || _isUav) && (isNull R3F_LOG_joueur_deplace_objet) &&
				...
				_objet_deverrouille && 	VEHICLE_UNLOCKED(_objet_pointe) && !(_objet_pointe getVariable "R3F_LOG_disabled");
			...
			R3F_LOG_action_revendre_usine_deplace_valide = _usine_autorisee_client && R3F_LOG_CFG_CF_sell_back_bargain_rate != -1 &&
				...
				(R3F_LOG_joueur_deplace_objet == _objet_pointe) && VEHICLE_UNLOCKED(_objet_pointe) && !(_objet_pointe getVariable "R3F_LOG_disabled") && !isNull _objet_pointe_autre_que_deplace &&
				{
		...
		if (_fonctionnalites select __can_be_towed) then
		{
			...
			if (_fonctionnalites select __can_be_moved_by_player) then
			{
				...
				R3F_LOG_action_remorquer_deplace_valide = VEHICLE_UNLOCKED(_objet_pointe) && !(_objet_pointe getVariable "R3F_LOG_disabled") && (count crew _objet_pointe == 0 || _isUav) &&
					(R3F_LOG_joueur_deplace_objet == _objet_pointe) && !isNull _objet_pointe_autre_que_deplace &&
					{
						...
						_objet_pointe_autre_que_deplace_deverrouille && VEHICLE_UNLOCKED(_objet_pointe_autre_que_deplace) && !(_objet_pointe_autre_que_deplace getVariable "R3F_LOG_disabled")
					};
			};
			...
			R3F_LOG_action_remorquer_direct_valide = (count crew _objet_pointe == 0 || _isUav) && isNull R3F_LOG_joueur_deplace_objet &&
				...
				_objet_pas_en_cours_de_deplacement && _objet_deverrouille && VEHICLE_UNLOCKED(_objet_pointe) && !(_objet_pointe getVariable "R3F_LOG_disabled") &&
				{
					{
						...
						!([_x, _joueur] call R3F_LOG_FNCT_objet_est_verrouille) && VEHICLE_UNLOCKED(_x) && !(_x getVariable "R3F_LOG_disabled") &&
						{
							private ["_delta_pos"];
			...
			R3F_LOG_action_detacher_valide = (isNull R3F_LOG_joueur_deplace_objet) &&
				...
				{VEHICLE_UNLOCKED(_objet_pointe getVariable "R3F_LOG_est_transporte_par")}
				} &&
		...
		if (_fonctionnalites select __can_transport_cargo) then
		{
			...
			R3F_LOG_action_charger_deplace_valide = alive _objet_pointe && (!isNull R3F_LOG_joueur_deplace_objet) &&
				...
				(vectorMagnitude velocity _objet_pointe < 6) && _objet_deverrouille && 	VEHICLE_UNLOCKED(_objet_pointe) && !(_objet_pointe getVariable "R3F_LOG_disabled");
		...
		R3F_LOG_action_deverrouiller_valide = _objet_pas_en_cours_de_deplacement && !_objet_deverrouille && VEHICLE_UNLOCKED(_objet_pointe) && !(_objet_pointe getVariable "R3F_LOG_disabled"); // BY VLADOS
	}
	else
	{
		R3F_LOG_action_deplacer_objet_valide = false;
		R3F_LOG_action_deplacer_objet_valide_enemy = false;
		R3F_LOG_action_deplacer_objet_valide_not_busy = false;
	...
	) then
	{
			...
			{
				{
					...
					VEHICLE_UNLOCKED(_x) &&
					(({(isPlayer _x && alive _x && _x distance _liftVeh < 100 && _x getVariable "mutex_net_obj" isEqualTo netId _liftVeh)} count (playableUnits) < 1) && !(_liftVeh getVariable ["objectLocked", false])) &&
					_x != _vehicule_joueur && !(_x getVariable "R3F_LOG_disabled") &&
					!(_x isKindOf "CUP_I_ZUBR_UN") &&
		...
		R3F_LOG_action_deplacer_objet_valide_enemy_heli = (({ (isPlayer _x && alive _x && _x distance player < 20) && !([_x, player] call A3W_fnc_isFriendly) } count playableUnits) < 1 && { (_x getVariable ["is_base_flag_activate", false]) && ( !( (_x getVariable ["ownerUID","0"] isEqualTo getPlayerUID player) || (_x getVariable ["ownerUID", "0"] in ((units player) apply {getPlayerUID _x})) || (group _x == group player) || ((side _x == side player) && (str(side player) == "WEST" || str(side player) == "EAST")) || ((str(side player) == "WEST" || str(side player) == "EAST") && (str(side player)) == (_x getVariable ["LastSide", ""])) ) || ((str(side player)) != _x getVariable ["LastSide", (str(side player))])) } count (player nearObjects ["FlagChecked_F", 320]) < 1);	
	}
	else
	{
		R3F_LOG_action_heliporter_valide = false;
		R3F_LOG_action_heliport_larguer_valide = false;
		R3F_LOG_action_deplacer_objet_valide_enemy_heli = false;
	};
