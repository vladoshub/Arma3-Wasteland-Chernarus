...
MF_ITEMS_SACK_FOOD = "sackfood";
MF_ITEMS_TEAPOT_WATER = "teapotwater";

//CUP
[MF_ITEMS_SACK_FOOD, "Sack of Food", {50 call mf_items_survival_eat}, "Land_Sack_F","client\icons\cannedfood.paa", 2] call mf_inventory_create;
[MF_ITEMS_TEAPOT_WATER, "TeaPot with Water", {50 call mf_items_survival_drink}, "Land_Teapot_EP1","client\icons\waterbottle.paa", 2] call mf_inventory_create;
...
_condition = "{_x getVariable ['food', 0] >= 1} count nearestObjects [player, ['Land_Sacks_goods_F'], 3] > 0 && !(MF_ITEMS_CANNED_FOOD call mf_inventory_is_full) && (vehicle player == player)";
_code =
{
...
_condition = "{_x getVariable ['water', 0] >= 1} count nearestObjects [player, ['Land_BarrelWater_F'], 3] > 0 && !(MF_ITEMS_WATER call mf_inventory_is_full) && (vehicle player == player)";
_code =
{
...
_label = "<img image='client\icons\water.paa'/> Fill Water Bottle";
_condition = "player distance cursorObject <= 3 && {{(str cursorObject) find _x != -1} count [': stallwater_f',': water_source_f'] > 0 && !(MF_ITEMS_WATER call mf_inventory_is_full)} && (vehicle player == player)";
_code =
{
	// player playMove ([player, "AmovMstpDnon_AinvMstpDnon", "putdown"] call getFullMove);
	player playActionNow "PutDown";
