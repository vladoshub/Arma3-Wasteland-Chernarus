...
drawPlayerIcons_thread = [] spawn
{
	...
	waitUntil
	{
		...
		if (!visibleMap && isNull findDisplay 49) then
		{
			...
			private _missionVehicleIcon = true;
			if (_missionVehicleIcon) then {
				//https://community.bistudio.com/wiki/Arma_3:_CfgMarkers
				{ 
					if(_x getVariable ["MissionVehicleDrawIcon", false]) then  { 
						_newArray pushBack [["\a3\ui_f\data\igui\cfg\simpletasks\types\move_ca.paa", [1, 1, 1, 1], ASLToAGL getPosASLVisual _x, 0.5, 0.5, 0, "Capture", 0], _x, 1 call drawPlayerIcons_posCode];
	 				};
 				}  forEach (player nearEntities ["AllVehicles", 700]);
			};
			...
			if (_noBuiltInThermal || (currentWeapon player) select [0,15] == "Laserdesignator") then
			{
				...
				if (_thermalActive || !isNil "A3W_builtInThermalOffline") then
				{
					...
					{
						//if (isNil "A3W_builtInThermalOffline") then BY_VLADOS
						//{
