...
#define MISSION_AI_FAR_DISTANCE 50 //75
...
A3W_mapDraw_thread = [] spawn
{
	...
	_atmIconSize = {(0.3 / ctrlMapScale (_this select 0)) max 9 min 24};
	...
	waitUntil
	{
		...
		if (!isNil "A3W_atmArray" && _drawAtmIcons) then
		{
...

			{
				if (alive _x) then
				{
					_newArrayIcons pushBack [[_atmIcon, [1,1,1,1], getPosWorld _x, _atmIconSize, nil, 0, "ATM", 0, 0.04], objNull];
				}
				else
				{
		...
		{
			{
				if (side _x == CIVILIAN) then
				{
					...
					_minDist = MISSION_AI_FAR_DISTANCE;
					if (["mission_DefendTerritory", _markerName] call fn_startsWith) then
					{
						_minDist = 200;
					};
					...
					if !(_markerPos isEqualTo [0,0,0]) then
					{
						_vehs = [];
						{
							...
							{
								if (alive _veh && _veh distance _markerPos > _minDist) then
								{
