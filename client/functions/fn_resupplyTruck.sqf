...
_resupplyThread = [_vehicle, _unit] spawn
{
	...
	_price = 3000; // price = 3000 for vehicles not found in vehicle store
	/*
	if (_vehicle isKindOf "B_SAM_System_02_F") then {
		_price = 3000;
		if ((player getVariable [(player getVariable ["playerPayType", "bmoney"]), 0]) < _price) exitWith {
	...
	*/
...

	{
		...
		{
			_price = _x select 2;
			//_price = round (_price / PRICE_RELATIONSHIP); //BY VLADOS
			if(_price > 900000) then {
				_price = ceil (_price / 20);
			} else {
	...
	_checkPlayerMoney =
	{
		if ((player getVariable [(player getVariable ["playerPayType", "bmoney"]), 0]) < _price) then
		{
