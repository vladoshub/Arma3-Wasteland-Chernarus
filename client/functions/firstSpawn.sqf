...
if (["A3W_combatAbortDelay", 0] call getPublicVar > 0) then
{
	...
	player addEventHandler ["Hit",
	{
		...
		//BY VLADOS ARMA2
		if (isNull objectParent player && alive player && diag_tickTime > (player getVariable ["localSoundHit", 0]) && _damage > 0.2) then {
		_localSoundHit = selectRandom ["georgeHit01", "georgeHit02", "georgeHit04", "georgeHit05", "georgeHit06", "jennikHit01", "jennikHit02", "jennikHit03", "jennikHit04", "jennikHit06", "vasaHit01", "vasaHit02", "vasaHit03", "vasaHit04", "vasaHit05", "vasaHit06", "vasaHit07", "vasaHit08", "vasaHit09"];

		[player, _localSoundHit] remoteExec ["global_say_3d", 2];

		player setVariable ["localSoundHit",  (diag_tickTime + 3), true];
	};
...
//["After the restart, you received weapons and equipment one-time. Next time you spawn you'll have to look for it yourself", 5] call mf_notify_client;
