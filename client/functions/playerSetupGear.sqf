...
// Remove NVG
//if (hmd _player != "" && (_player getVariable ["firstNVGogglesNeedRemove", true])) then { _player unlinkItem hmd _player };


//_player setVariable ["firstNVGogglesNeedRemove", true, true];
...
if(missionNamespace getVariable [_playerGlobalNotFirstInit, true]) then {
	...
	[player, 25000] call A3W_fnc_setCMoney;
...
*/
	...
	if (hmd _player == "") then { 
		_player linkItem "NVGoggles"; 
		//_player setVariable ["firstNVGogglesNeedRemove", false, true]; 
	};
	/*
...
_player addItem "Chemlight_yellow";

_player addItem "FirstAidKit";

if (!([_player, "ItemMap"] call BIS_fnc_hasItem)) then {
