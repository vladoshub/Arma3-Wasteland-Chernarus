...
storePurchaseHandle = _this spawn
{
	...
	if !([_itemData] call A3W_fnc_isStoreItemAvailable) exitWith { hint "This vehicle is no longer available in the current store rotation. Reopen the store."; };
	...
	if (!isNil "_price" && {_price > -1}) then // vehicle price now handled in spawnStoreObject.sqf
	{
		...
		_playerMoneyText ctrlSetText format ["Money: $%1", [player getVariable [(player getVariable ["playerPayType", "bmoney"]), 0]] call fn_numbersText];
