...
private ["_payType", "_playerMoneyCurrent", "_newBalance"];
...
storeSellingHandle = _this spawn
{
	...
	if (_forceSell) then
	{
		...
		_payType = player getVariable ["playerPayType", "bmoney"];
		_playerMoneyCurrent = player getVariable [_payType, 0];
		if(_payType isEqualTo "bmoney") then {
			_newBalance = _playerMoneyCurrent + _sellValue;
		...
		if ([parseText _confirmMsg, "Confirm", "Sell", true] call BIS_fnc_guiMessage) then
		{
			...
			if (CARGO_STRING(_crate) == _originalCargo) then
			{
				...
				if (_deleteObject) then
				{
					...
					{
						[_crate, player, -1, false] execVM "R3F_LOG\objet_deplacable\relacherWithOutCheck.sqf";
