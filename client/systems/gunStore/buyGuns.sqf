...
if (!isNil "gunStore_lastPurchaseTime") then //BY VLADOS
{
	private _timeLeft = (15 - (diag_tickTime - gunStore_lastPurchaseTime));

	if (_timeLeft > 0) then
	{
		hint format ["You need to wait %1s before buying another gun", ceil _timeLeft];
...
storePurchaseHandle = _this spawn
{
			...
			{
				{
					if (_itemData == _x select 1) exitWith
					{
						...
						{
							if ((_class == "hgun_esd_01_F") || !([player, _class] call addWeaponInventory)) then 
							{
			...
			{
				{
					...
					if (_itemData == _x select 1) exitWith
					{
						...
						gunStore_lastPurchaseTime = diag_tickTime;
	...
	if (!isNil "_price" && {_price > -1}) then
	{
		...
		if (isNil "_requestKey" && _price > player getVariable [(player getVariable ["playerPayType", "bmoney"]), 0]) exitWith
		{
			[_itemText] call _showInsufficientFundsError;
		};
		...
		if((player getVariable ["playerPayType", "bmoney"]) isEqualTo "bmoney") then {
			if (isNil "_requestKey") then // static gun price now handled in spawnStoreObject.sqf
			{
				...
				_playerMoneyText ctrlSetText format ["Balance: $%1", [_newBalance] call fn_numbersText];
			};
		} else 
		{
			if (isNil "_requestKey") then // static gun price now handled in spawnStoreObject.sqf
