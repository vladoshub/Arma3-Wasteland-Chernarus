...
// ******************************************************************************************
//	@file Version: 1.0
//	@file Name: populateCustomLoad.sqf
...
_balance ctrlSetText format ["Balance: $%1", [player getVariable ["bmoney",0]] call fn_numbersText]; //BY_VLADOS
...
switch(_switch) do
{
	...
	case 11:
	{
		...
		_excludedItems pushBack "stash";
		_excludedItems pushBack "cannedfood";
		_excludedItems pushBack "camonet";
		_excludedItems pushBack "syphonhose";
		_excludedItems pushBack "quad";
		_excludedItems pushBack "car";
		_excludedItems pushBack "artillery";
...
private _dynamicStoreSection = switch (_switch) do
{
    ...
    case 10: { "genItemArray" };
    case 11: { "customPlayerItems" };
    case 12: { "gasItemArray" };
    default { "" };
};
...
if (_wepFilter) then
{
	...
	_wepFilterDropdown ctrlAddEventHandler ["LBSelChanged", { [7] call populateCustomLoad }];
};
...

{
	...
	if (!isNil "_parentCfg") then
	{
		...
		_gunlistIndex = _gunlist lbAdd format ["%1", if (!isNil "_parentCfg" && _x select 0 == "") then { getText (_parentCfg >> _weaponClass >> "displayName") } else { _x select 0 }];
		_gunlist lbSetPicture [_gunlistIndex, _x select 3];
		_gunlist lbSetData [_gunlistIndex, _weaponClass];
	};
} forEach _itemsArray;