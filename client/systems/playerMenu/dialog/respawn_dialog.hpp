...
class RespawnSelectionDialog
{
	...
	class Controls
	{
		class RspnRandomButton: RspnButton
		{
			...
			//onLoad = "(_this select 0) spawn {_button = _this; while {!isNull _button} do {_button ctrlSetBackgroundColor [1,0,0,1]; sleep 0.5; _button ctrlSetBackgroundColor [0.3,0,0,0.7]; sleep 0.5;}};";
		...
		class cusLoadoutButton: RspnButton
 		{
 			idc = custom_Loadout_Button;
			//colorBackground[] = {1,0,0,0.5};
 			onButtonClick = "[] execVM 'client\systems\customLoad\customLoad.sqf'";
			tooltip = "You can purchase weapons and loot and spawn with them. Selling is not an option. Also, if you fill your backpack, you'll spawn without a starter pack.";
 			text = "Buy weapons before spawn";
 			default = true;
 			
 			#define customButton_W (0.2 * X_SCALE)
 			#define customButton_X RspnLine_X
 
 			x = customButton_X + 0.6;
 			y = RspnRandomButton_Y;
 			w = customButton_W + 0.1;
		};
...
*/
		...
		class RspnSpawnButton: RspnButton
		{
			...
			onLoad = "(_this select 0) spawn {_button = _this; while {!isNull _button} do {_button ctrlSetBackgroundColor [1,0,0,1]; sleep 1; _button ctrlSetBackgroundColor [0.3,0,0,0.7]; sleep 1;}};";
