...
if (!isNil "genStore_lastPurchaseTime") then //BY VLADOS
{
	private _timeLeft = (1 - (diag_tickTime - genStore_lastPurchaseTime));

	if (_timeLeft > 0) then
	{
		hint format ["You need to wait %1s before buying another item", ceil _timeLeft];
	...
	{
		{
			if (_itemData == _x select 1) exitWith
			{
				...
				genStore_lastPurchaseTime = diag_tickTime;
...
} forEach allUnitsUAV;*/ //BY VLADDOS B_AAA
