...
if (!isNil "_getPublicVar" && !isNil "_isConfigOn") then
{
	   ...
	   {!(["playerSpawning", false] call _getPublicVar)}) then
	{
		...
		if (player getVariable ["cmoney",0] >= 200000) then {
			private _display = findDisplay 49;
			if (!isNull _display) then
			{
				(_display displayCtrl 104) ctrlEnable false; // Abort
			};
			systemChat "Cannot leave with more then 200000 Cash! Go to a Bank";
		} else {
			private _display = findDisplay 49;
			if (!isNull _display) then
			{
		...
		if (_abortDelay > 0) then
		{
			...
			if !(["onPauseLoopRunning", false] call _getPublicVar) then
			{
				...
				while {!isNull findDisplay 49} do
				{
					if (call _preventAbort) then
					{
						//false call _enableButtons; //BY_VLADOS
						...
						waitUntil
						{
							if (diag_tickTime - _time >= 1) then
							{
								...
								{
									_text = "\n\n\n\nCannot  out during bleeding!";
								}
								else
								{
									_timer = with missionNamespace do { (_abortDelay - (diag_tickTime - _timeStamp)) call fn_formatTimer };
									_text = format ["\nCannot out during combat! (%1)", _timer];
								};

								10290 cutText [_text, "PLAIN DOWN"]; // 10290 is higher than ReviveGUI_IDD //EDIT BY_VLADOS
								_time = diag_tickTime;
							};
