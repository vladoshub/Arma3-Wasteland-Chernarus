...
_respawnPos = markerPos (selectRandom _respawnMarkers);
...
if !(_respawnPos isEqualTo [0,0,0]) then
{
	...
	if (surfaceIsWater _respawnPos) then
	{
		...
		_buildings = (lineIntersectsSurfaces [_top, _respawnPos, objNull, objNull, true, -1, "GEOM", "NONE"]) select {(_x select 2) isKindOf "Land_Pier_Box_F"}; //Check respawn Vlados //Land_PierConcrete_01_16m_F
