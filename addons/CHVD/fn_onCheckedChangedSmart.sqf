...
_elOne = param [5, controlNull, [0, controlNull]];
_elTwo = param [6,  controlNull, [0, controlNull]];
_elThree = param [7,  controlNull, [0, controlNull]];
_elFour = param [8, controlNull, [0, controlNull]];
...
if (_state == 1) then {
	...
	*/
	...
	ctrlEnable [_elOne, true];
	ctrlEnable [_elTwo, true];
	ctrlEnable [_elThree, true];
	ctrlEnable [_elFour, true];

	call compile format ["%1 = true",_syncVar];
	call compile format ["profileNamespace setVariable ['%1',%1]", _syncVar];

	ctrlSetText [_text, str round ((sliderPosition _slider) min CHVD_maxObj max 0)];
	sliderSetPosition [_slider, (sliderPosition _slider) min CHVD_maxObj max 0];

	call compile format ["%1 = %2", _varType, (sliderPosition _slider) min CHVD_maxObj max 0];
	call compile format ["profileNamespace setVariable ['%1',%1]", _varType];
	...
	ctrlEnable [_elOne, false];
	ctrlEnable [_elTwo, false];
	ctrlEnable [_elThree, false];
	ctrlEnable [_elFour, false];
};
