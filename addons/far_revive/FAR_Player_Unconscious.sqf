...
if (_killer != _unit && isPlayer _killer && !([_killer, _unit] call A3W_fnc_isFriendly)) then
{
	private _currentPlayerSide = playerSide;
	private _friendlyFlags = _unit nearObjects ["FlagChecked_F", 300] select { ( (_x getVariable ["is_base_flag_activate", false]) && ((str(_currentPlayerSide)) == (_x getVariable ["LastSide", (str(_currentPlayerSide))])) && ((_x getVariable ["ownerUID","0"] isEqualTo getPlayerUID _unit) || (_x getVariable ["ownerUID", "0"] in ((units _unit) apply {getPlayerUID _x})) || (group _x == group _unit) || ((side _x == _currentPlayerSide) && (str(_currentPlayerSide) == "WEST" || str(_currentPlayerSide) == "EAST")) || ((str(_currentPlayerSide) == "WEST" || str(_currentPlayerSide) == "EAST") && (str(_currentPlayerSide)) == (_x getVariable ["LastSide", ""]))) ) };
	private _nearFlagsCount = count (_friendlyFlags); 

	if(_nearFlagsCount == 1) then {
		[[_unit, _killer, _currentPlayerSide],"Base_flag_srv_killed",false,false,false] call BIS_fnc_MP;
	};

	_unit setVariable ["killOnBaseFlag", true, true];
