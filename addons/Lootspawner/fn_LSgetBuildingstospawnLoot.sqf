				...
				if ((floor random 100) < _chpSpot) then {
					...
					if ((count (nearestObjects [_spwnPos, LSusedclass_list, 0.5])) == 0) then {
						...
						switch (_lootType) do
						{
							...
							case 1:
							{
								_loot = selectRandom ((lootWeapon_list select _lootClass) select 1);
							...
							case 2:
							{
								...
								for "_rm" from 1 to _randChance do {
									_loot = selectRandom ((lootMagazine_list select _lootClass) select 1);
							...
							case 3:
							{
								_loot = selectRandom ((lootItem_list select _lootClass) select 1);
								_lootholder addItemCargoGlobal [_loot, 1];
							};
							//special for backpacks
							case 4:
							{
								_loot = selectRandom ((lootBackpack_list select _lootClass) select 1);
								_lootholder addBackpackCargoGlobal [_loot, 1];
							};
							//special for world objects: account for Wasteland and other items
							case 5:
							{
								_loot = selectRandom ((lootworldObject_list select _lootClass) select 1);
