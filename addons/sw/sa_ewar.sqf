...
sa_ins_list=["B_UAV_03_dynamicLoadout_F","B_UAV_05_F","B_UAV_02_dynamicLoadout_F","O_UAV_02_dynamicLoadout_F","O_T_UAV_04_CAS_F","I_UAV_02_dynamicLoadout_F", "I_UAV_01_backpack_F", "O_UAV_01_backpack_F", "B_UAV_01_backpack_F"];//UAVs classes with inertial navigation (dont lose waypoints after jamm&start operate acording waypoint program) //last 3 UAV added by vlados (01_backpack_F)
	...
	_weak_signals=0;
	{
		...
		if((_x select 1)>sa_ident_str) then {
			...
			if(([_x] call sa_is_signal_uav)>0) then {
				...
				if (((_x select 0 select 0) getVariable ["ownerUID","0"] isEqualTo getPlayerUID player) || ((_x select 0 select 0) getVariable ["ownerUID", "0"] in ((units player) apply {getPlayerUID _x})) || (group (_x select 0 select 0) == group player) || ((side (_x select 0 select 0) == side player) && (str(side player) == "WEST" || str(side player) == "EAST"))) then {
...
fnc_sa_local_add_to_jamm_list={
	...
	//if (side _unit==side player) then {	sa_local_jamm_buffer pushBackUnique _unit;};
	  if(((_unit) getVariable ["ownerUID","0"] isEqualTo getPlayerUID player) || ((_unit) getVariable ["ownerUID", "0"] in ((units player) apply {getPlayerUID _x})) || (group _unit == group player) || ((side (_unit) == side player) && (str(side player) == "WEST" || str(side player) == "EAST"))) then 
	  {	
