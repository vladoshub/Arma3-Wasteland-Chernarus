...
pp_get_near_vehicles = {
  ...
  def(_vehicles);
  _vehicles = (nearestObjects [_player, ["LandVehicle","Air","Ship"], 50]) select {!(_x getVariable ["A3W_lockpickDisabled",false]) && !(_x getVariable ["objectLocked", false]) && (locked _x) < 2 && ((count crew _x == 0) || (getNumber (configFile >> "CfgVehicles" >> (typeOf _x) >> "isUav") == 1))}; //BY VLADOS
...
pp_retrieve_transaction_ok = {
  ...
  def(_cmoney);
  _cmoney = _player getVariable [(player getVariable ["playerPayType", "bmoney"]), 0];
  ...
  if((player getVariable ["playerPayType", "bmoney"]) isEqualTo "bmoney") then {
    private _playerMoney = player getVariable ["bmoney", 0];
		private _newBalance = _playerMoney - _cost;
...
pp_park_vehicle_action = {
  ...
  if (_vehicle getVariable ["A3W_notForParking", false]) exitWith {
    _player groupChat format["%1, this vehicle is not allowed to be parked.", (name _player)];
  };
...
pp_is_player_near = {
  private["_objects"];
  _objects = nearestObjects [player, ["Land_CampingTable_small_F", "Land_i_Garage_V1_F"], 2];
