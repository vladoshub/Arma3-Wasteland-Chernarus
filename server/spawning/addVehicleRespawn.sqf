...
_respawnTimer =   PARAM(2, 5*60);  // Default respawn timer when vehicle is broken (0 = respawn immediatly (not recommended))
_desertedTimer =  PARAM(3, 10*60);  // Default respawn timer when vehicle is deserted but not broken
_proxyTimer =     PARAM(4, 25*60);  // Respawn timer when the last vehicle owner is within the defined proximity distance
