...
A3W_spawnBeaconSpawnHeight = 500; // Altitude in meters at which players will spawn when using spawn beacons (0 = ground/sea)
...
A3W_atmMaxBalance = 900000000;       // Maximum amount of money that can be stored in a bank account (don't go over 16777216 as numbers start losing accuracy)
...
A3W_artilleryShells = 3;          // Number of shells per strike
A3W_artilleryRadius = 50;          // Strike radius in meters within which the shells will randomly hit
A3W_artilleryCooldown = 150*60;     // Cooldown in seconds between strike uses per individual player - resets on server …
A3W_artilleryCrateOdds = 0.01;     // Odds of finding one Artillery Strike in a mission crate (0.0 to 1.0)
A3W_artilleryAmmo = "R_230mm_fly"; //"ModuleOrdnanceHowitzer_F_ammo"; // CfgAmmo class of fired shells, also available for example are the more powerful "R_230mm_fly" and less powerful "ModuleOrdnanceMortar_F_ammo"
