...
private _objList = call compile preprocessFileLineNumbers format ["server\missions\ultraMissions\baseBlocks\%1.sqf", _fileName];
private _objs = [];

{
	...
	private _objInit = _x param [3, nil];
	private _objPos = _pos vectorAdd ([_objPos, -(_dir)] call BIS_fnc_rotateVector2D);
	private _obj = createVehicle [_objName, _objPos, [], 0, "CAN_COLLIDE"];
	_obj setDir (_dir + _objDir);
	_obj setPos _objPos;
	_obj setPosATL _objPos;
	private _objDirCurr = getDir _obj;
	_objs pushBack _obj;
