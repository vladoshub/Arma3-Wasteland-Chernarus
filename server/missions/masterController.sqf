...
[
	...
	"extraMission",
	"sideMission",
	"ultraMission" //BY VLADOS
];

//private _missionOnMap = count (allMapMarkers select {markerType _x isEqualTo "mil_destroy" && {[["mission_"], _x] call fn_startsWith}});
			...
			for "_i" from 0 to 1 step 0 do { //ARYX
				while {isNil "_current"} do {
					...
					if (count _availables > 0) then {
						_current = selectRandom _availables;
