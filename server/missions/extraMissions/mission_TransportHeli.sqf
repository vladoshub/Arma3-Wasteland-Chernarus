...
_setupVars = {
	_missionType = "MIDDLE: Helicopter Escort";
...
_setupObjects = {
	...
	if ((floor random 2) isEqualTo 1 ) then {
		_veh1 append [(selectRandom _transportChoicesArmed)];
		_veh1 append [true];
	}
	else {
		_veh1 append [(selectRandom _transportChoicesUnarmed)];
		_veh1 append [false];
	};

	_veh2 = selectRandom _supportChoices;
	_veh3 = selectRandom _supportChoices;
