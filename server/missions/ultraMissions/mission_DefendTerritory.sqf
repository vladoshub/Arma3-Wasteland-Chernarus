...
_setupVars =
{
	_missionType = "ULTIMATE: Defend Territory";
	...
	_territory = selectRandom _territories;
	while {!(["TERRITORY_TAKE_MISSION", _territory select 0] call fn_startsWith)} do
	{
		_territory = selectRandom _territories;
	};
	...
	_reward = (_territory select 2) * 40 max 500000;
};
...
_setupObjects =
{
	_createVehicle =
	{
		...
		_soldier = [_group, _position, _camo] call createRandomSoldierNew;
	...
	_spawnWave =
	{
		...
		_minDist = (selectMax _triggerArea) * 1.5 min 150;
		...
		if (_ranOff == 0) then
		{
			...
			_apcClass = selectRandom ["B_APC_Tracked_01_rcws_F", "I_APC_tracked_03_cannon_F", "O_APC_Tracked_02_cannon_F"];
		...
		if (_ranOff == 1) then
		{
			...
			_tankClass = selectRandom ["B_MBT_01_cannon_F", "O_MBT_02_cannon_F", "I_MBT_03_cannon_F"];
		...
		if (_ranOff == 2) then
		{
			...
			_aaClass = selectRandom ["B_APC_Tracked_01_AA_F", "O_APC_Tracked_02_AA_F", "I_LT_01_AA_F"];
		...
		_randomBox = selectRandom ["mission_USRifles", "mission_RURifles", "mission_Explosive"];
		...
		{
			private _attachItem = createVehicle ["SmokeShellBlue", getPosATL _para, [], 0, "NONE"];
	...
	_camo = selectRandom ["MTP", "Tropic", "CTRGUrban", "CTRGArid", "CTRGTropic", "Woodland", "GreenHex", "Hex", "Green", "Taiga", "Digital", "Geometric", "Guerilla"];
	_camo = selectRandom ["Tropic", "Woodland", "GreenHex", "Hex", "Green", "Guerilla"];
	...
	_totalAttacker = ((1 + (count allPlayers)) * 30);
	...
	_missionHintText = format ["<t color='%1'>%2</t> will be infiltrate by enemy, defend and capture it!", ultraMissionColor, _territoryName];
};
...
_failedExec =
{
	...
	_triggerArea = triggerArea _trigger;
	private _playerCoef = (1 + (count (allPlayers select { _x inArea [position _trigger, _triggerArea select 0 * 1.5, _triggerArea select 1 * 1.5, _triggerArea select 2, _triggerArea select 3, -1] })) / 2);



	_totalDead = count ((_trigger getVariable ["Attackers", []]) select {!alive _x});
	_failReward = [((_reward * (_totalDead / 100 )) / _playerCoef), 0] call BIS_fnc_cutDecimals;
	if (_failReward > 205000) then {
		_failReward = 205000;
	};
...
_successExec =
{
	...
	_box3 = createVehicle ["Box_FIA_Support_F", (position _trigger) vectorAdd [random 30 + 35, random 30 - 15, random 30 + 100], [], 0, "NONE"];
	{
		...
		[_x, selectRandom _randomBox] call fn_refillbox;
		_x setVariable ["R3F_LOG_disabled", false, true];
		_x setVariable ["cmoney", _totalAttacker * 300, true];
		...
		if (dayTime <= 20.5 && dayTime > 3.5) then
		{
			private _attachItem = createVehicle ["SmokeShellBlue", getPosATL _x, [], 0, "NONE"];
