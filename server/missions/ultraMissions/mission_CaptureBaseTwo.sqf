...
_setupVars =
{
	_missionType = "HARD: Base Capture";	
...
_setupObjects =
{
	...
	_createVehicle =
	{
		private ["_type", "_position", "_direction", "_variant", "_special", "_vehicle", "_soldier", "_soldierGun"];
	...
	private _vehChoices =
	[
		["I_LT_01_AT_F", "B_MRAP_01_gmg_F"],
		["O_APC_Tracked_02_cannon_F", "I_C_Offroad_02_AT_F"]
	];

	if(["A3W_use_CUP", false] call getPublicVar) then {
		_vehChoices append [
		[(CUP_Classes select {_x == "CUP_B_BMP2_CDF"}) select 0, (CUP_Classes select {_x == "CUP_B_M1167_USA"}) select 0],
		[(CUP_Classes select {_x == "CUP_B_UAZ_METIS_CDF"}) select 0, (CUP_Classes select {_x == "CUP_O_BTR90_RU"}) select 0]
		];
	};
	...
	_block = selectRandom (call compile preprocessFileLineNumbers "server\missions\ultraMissions\baseBlocks\baseBlockListTwo.sqf");
	_aiGroup = createGroup CIVILIAN;
	[_aiGroup, _missionPos, _nbUnits, 30] call createCustomGroupAA;
	...
	{ 	
		...
		{
			private _soldierCommander = [_aiGroup, _missionPos] call createRandomSoldierC;
	...
	*/
	...
	_missionHintText = format ["The enemies created a base! Take them!", ultraMissionColor];
};
...
_successExec =
{
	...
	uiSleep 1;
		...
		private _box1 = createVehicle ["Box_East_WpsSpecial_F", _missionPos, [], 5, "None"];
		_box1 setDir random 360;
		[_box1, "mission_Launchers1"] call fn_refillbox;

		private _box2 = createVehicle ["Box_T_East_WpsSpecial_F", _missionPos, [], 5, "None"];
