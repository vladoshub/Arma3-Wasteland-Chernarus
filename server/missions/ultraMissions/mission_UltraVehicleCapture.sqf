...
private ["_vehicle", "_vehicleName", "_vehDeterminer", "_variant", "_veh1Object", "_createVehicle", "_vehicleClass", "_vehChoices", "_vehChoicesAT", "_veh2Object"];
...
_setupObjects =
{
	...
	if(["A3W_use_CUP", false] call getPublicVar) then {
		...
		[
		    ...
		    (CUP_Classes select {_x == "CUP_B_HMMWV_Avenger_USMC"}),
	...
	*/
	...
	if(random 1 < 0.35) then {
		...
		_vehChoices append 
		[
			(CUP_Classes select {_x == "CUP_O_2S6M_RU"})
		];
	};	 
	};
	...
	if(["A3W_use_CUP", false] call getPublicVar) then {
		_vehChoicesAT append [
			(CUP_Classes select {_x == "CUP_B_BMP2_CDF"}),
			(CUP_Classes select {_x == "CUP_B_UAZ_METIS_CDF"}),
			(CUP_Classes select {_x == "CUP_B_HMMWV_Avenger_USMC"}),
			(CUP_Classes select {_x == "CUP_O_BMP3_RU"}),
			(CUP_Classes select {_x == "CUP_B_M7Bradley_USA_D"}),
			(CUP_Classes select {_x == "CUP_O_BTR90_RU"})
		];
	};
	...
	private _ultraVehRandomAT = selectRandom _vehChoicesAT;

	private _veh1 = _ultraVehRandom select 0;
	private _veh2 = _ultraVehRandomAT select 0;
	...
	[_aiGroup, _missionPos, _nbUnits] call createCustomGroupAA;


	private _spawn1 = [(_missionPos select 0), ((_missionPos select 1) + 35), (_missionPos select 2)];
	private _spawn2 = [((_missionPos select 0) + 35), (_missionPos select 1), (_missionPos select 2)];
