...
private ["_vehicle", "_vehicleName", "_vehDeterminer", "_variant", "_veh1Object", "_veh2Object", "_vehicleClass", "_createVehicle"];
...
_setupObjects =
{
	...
	private _missionWaterPos = [_missionPos select 0, ((_missionPos select 1) - 400), _missionPos select 2]; //was 200

	// Class, Position, Fuel, Ammo, Damage, Special
	_vehicle = [_vehicleClass, _missionWaterPos] call createMissionVehicle;
	...
	[
		...
		["I_LT_01_AA_F", "I_LT_01_AA_F"],
		["B_MBT_01_TUSK_F", "B_APC_Tracked_01_AA_F"]
	];

	if(["A3W_use_CUP", false] call getPublicVar) then {
		_vehChoices append [
		[(CUP_Classes select {_x == "CUP_B_M1A1_DES_US_Army"}) select 0, (CUP_Classes select {_x == "CUP_B_UAZ_AA_CDF"}) select 0],
		[(CUP_Classes select {_x == "CUP_O_BMP3_RU"}) select 0, (CUP_Classes select {_x == "CUP_B_HMMWV_Avenger_USMC"}) select 0],
		[(CUP_Classes select {_x == "CUP_O_T90_RU"}) select 0, (CUP_Classes select {_x == "CUP_I_Datsun_AA_Random"}) select 0]
		];
	};
	...
	private _spawn1 = [((_missionPos select 0) + 20), (_missionPos select 1), (_missionPos select 2)];
	private _spawn2 = [(_missionPos select 0), ((_missionPos select 1) + 20), (_missionPos select 2)];
