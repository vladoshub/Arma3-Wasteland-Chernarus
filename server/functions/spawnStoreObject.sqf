...
private ["_isGenStore", "_isGunStore", "_isVehStore", "_timeoutKey", "_objectID", "_playerSide", "_objectsArray", "_results", "_itemEntry", "_itemPrice", "_safePos", "_object", "_payType", "_balance"];
...
if (_key != "" && _player isKindOf "Man" && {_isGenStore || _isGunStore || _isVehStore}) then
{
	...
	{
		if !([_class] call A3W_fnc_isStoreItemAvailable) exitWith
		{
		...
		_payType = _player getVariable ["playerPayType", "bmoney"];
		_balance = _player getVariable [_payType, 0];
		if (_player getVariable [_payType, 0] >= _itemPrice) then
		{
			...
			if(_class == "Land_PierConcrete_01_16m_F" || _class == "Land_Pier_Box_F") then {
				private _zPos = (_safePos select 2);
				_safePos set [2, _zPos + 2];
			};
			...
			if (_isUAV) then
			{
				...
				//{
					...
					private _grp = [_object, _playerSide, true] call fn_createCrewUAV; //TEST!

					if (isNull (_object getVariable ["ownerGroupUAV", grpNull])) then
					{
						_object setVariable ["ownerGroupUAV", _playerGroup, true]; // not currently used
					};
				//};
			};
			...
			if !(_player getVariable [_timeoutKey, true]) then
			{
				...
				{
					private _newBalance = _balance - _itemPrice;
			...
			if(_object isKindOf "Land_Sacks_goods_F") then  //BY VLADOS
			{
				_object setVariable ["food", 40, true];
			};

			if(_object isKindOf "Land_BarrelWater_F") then //BY VLADOS
			{
