...
if (hasInterface) then //BY VLADOS
{
	if(_veh isKindOf "C_Truck_02_box_F") then {
		...
		_veh addAction ["<img image='client\icons\repair.paa'/> Reammo Vehicle", "client\functions\fn_reammoTruck.sqf", [], 51, false, true, "", "alive _target && alive objectParent _this && (speed vehicle _this < 10) && (speed vehicle _this > -10) && (_target != vehicle _this) && _target distance vehicle _this <= (10 max (sizeOf typeOf vehicle _this * 0.75)) && (isNil 'mutexScriptInProgress' || {!mutexScriptInProgress})"]; // _target = truck, _this = player
	};
};
