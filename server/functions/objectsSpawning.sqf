...
private ["_pos", "_angleIncr", "_langle", "_lpos", "_obj"];
private _tradius = 0;
...
private _sradius = if (!_baseBuilding && _essentials) then { 15 } else { 15 };
...

{
	...
	while {_lcounter < _objammount} do
	{
		...
		private _objOne = selectRandom _objList;
		private _objProbability = _objOne select 0;
		private _currentRandom  = random 1;
		
		if(_objProbability > _currentRandom) then {

		private _typeOfObject = _objOne select 1;
		_lpos = _pos vectorAdd ([[_maxrad, 0, 0], _langle] call BIS_fnc_rotateVector2D);
		_spawnedObjects pushBack ([_lpos, _minrad, _maxrad, _counter, _typeOfObject] spawn _createRandomObject);
		//_minrad = _minrad + 15;
		//_maxrad = _maxrad + 15;
