...
//BY VLADOS BASE FLAGS VARS
//base_flag addon
private _non_unlock_mode = _obj getVariable "non_unlock_mode"; //for flags and nearest objects
if (!isNil "_non_unlock_mode") then
{
	_variables pushBack ["non_unlock_mode", _non_unlock_mode];
};

private _is_base_flag_activate = _obj getVariable "is_base_flag_activate"; //only flag
if (!isNil "_is_base_flag_activate") then
{
	_variables pushBack ["is_base_flag_activate", _is_base_flag_activate];
};

private _flag_respawn = _obj getVariable "flag_respawn"; //respawn on flag after restart
if (!isNil "_flag_respawn") then
{
	_variables pushBack ["flag_respawn", _flag_respawn];
};

private _secure_by_flag = _obj getVariable "secure_by_flag"; //no delete obj after restart
if (!isNil "_secure_by_flag") then
{
	_variables pushBack ["secure_by_flag", _secure_by_flag];
};

private _code_lock = _obj getVariable "theCode"; //code lock
if (!isNil "_code_lock") then
{
	_variables pushBack ["theCode", _code_lock];
};

private _lastSide = _obj getVariable "LastSide"; //code lock
if (!isNil "_lastSide") then
{
	_variables pushBack ["LastSide", _lastSide];
};

private _payBuild = _obj getVariable "payBuild";
if (!isNil "_payBuild") then
{
	_variables pushBack ["payBuild", _payBuild];
};

private _not_for_sale = _obj getVariable "A3W_notForSale";
if (!isNil "_not_for_sale") then
{
	_variables pushBack ["A3W_notForSale", _not_for_sale];
};
