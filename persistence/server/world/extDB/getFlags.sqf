...
_maxLifetime = ["A3W_objectLifetimeFlag", 0] call getPublicVar;
_saveUnlockedObjects = ["A3W_extDB_SaveUnlockedObjects", 0] call getPublicVar;

if (_maxLifetime > 0 || _saveUnlockedObjects <= 0) then
{
	//base_flag addon
	[format ["deleteExpiredServerObjectsBaseFlag:%1:%2:%3:%4", call A3W_extDB_ServerID, call A3W_extDB_MapID, _maxLifetime, _saveUnlockedObjects], 2, true] call extDB_Database_async;
};
...
_result = [format ["getServerFlags:%1:%2:%3", call A3W_extDB_ServerID, call A3W_extDB_MapID, _columns joinString ","], 2, true] call extDB_Database_async;
