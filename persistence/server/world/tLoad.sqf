...
if (["A3W_weatherSaving"] call isConfigOn) then 
{
	...
	(_time select [1,4]) execVM "addons\weather\RunExecutors.sqf";
	//drn_DynamicWeather_MainThread = (_time select [1,4]) execVM "addons\scripts\DynamicWeatherEffects.sqf"; //BY_VLADOS
};
